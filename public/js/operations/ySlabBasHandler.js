const ySlabBasOperation = (() => {
    ...
    async function loadData(BASE_URL, operation) {
        ...
        if (!result.success) throw new Error(result.error || 'Failed to load Slab Basis data');
        return result.data || [];
    }
    ...
    async function saveRow(row, uniqueId, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to update Slab Basis');
        return result.data;
    }
    ...
    async function saveNewRow(row, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to add Slab Basis');
        return result.data;
    }
    ...
    async function deleteRow(uniqueId, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to delete Slab Basis');
        return result.data;
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ySlabBasOperation;
}
