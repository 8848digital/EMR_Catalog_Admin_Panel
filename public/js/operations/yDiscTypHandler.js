const yDiscTypOperation = (() => {
    ...
    async function loadData(BASE_URL, operation) {
        ...
        if (!result.success) throw new Error(result.error || 'Failed to load Discount Type data');
        return result.data || [];
    }
    ...
    async function saveRow(row, uniqueId, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to update Discount Type');
        return result.data;
    }
    ...
    async function saveNewRow(row, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to add Discount Type');
        return result.data;
    }
    ...
    async function deleteRow(uniqueId, BASE_URL, operation) {
        ...
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to delete Discount Type');
        return result.data;
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = yDiscTypOperation;
}
