const csGrdOperation = (() => {
  const COLUMNS = [
    { key: 'PTyp', label: 'Type', hidden: true },
    { key: 'PMCd', label: 'CS Grade Code', editable: true, type: 'dropdown', width: '200px' },
  ...
  async function loadPMCdList(BASE_URL, operation) {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Failed to load CS Grade code list');
      }
  ...
  async function loadData(BASE_URL, operation) {
    ...
    if (!result.success) {
      throw new Error(result.error || 'Failed to load CS Grade data');
    }
  ...
  function createEditControl(col, value, row) {
    if (col.type === 'dropdown' && col.key === 'PMCd') {
      ...
      defaultOption.value = '';
      defaultOption.textContent = '--Select CS Grade--';
  ...
  async function saveRow(row, uniqueId, BASE_URL, operation) {
    ...
    if (!result.success) {
      throw new Error(result.error || 'Failed to update CS Grade');
    }
      ...
      if (col.editable && col.type === 'dropdown' && col.key === 'PMCd') {
        ...
        return `<td ${widthStyle}>
                  <select class="edit-field" data-field="${col.key}" 
                    ...
                    <option value="">--Select CS Grade--</option>
                    ${options}
                  </select>
                </td>`;
  ...
  async function saveNewRow(row, BASE_URL, operation) {
    ...
    if (!PMCd) {
      throw new Error('Please select CS Grade Code (PMCd)');
    }
    ...
    if (!result.success) {
      throw new Error(result.error || 'Failed to add CS Grade');
    }
  ...
  async function deleteRow(uniqueId, BASE_URL, operation) {
    ...
    if (!result.success) {
      throw new Error(result.error || 'Failed to delete CS Grade');
    }
