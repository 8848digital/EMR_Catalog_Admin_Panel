...
module.exports = {
        ...
        customUpdate: async (conn, body, modUsr) => {
            ...
            if (isPMCdChanged) {
                ...
                if (duplicateResult[0]?.Count > 0) {
                    throw new Error(`Diamond Grade Code "${newPMCd}" already exists`);
                }
            }
            ...
            if (result.rowsAffected[0] === 0) {
                throw new Error('Diamond Grade not found or no changes made');
            }

            return {
                message: 'Diamond Grade updated successfully',
                PMCd: newPMCd,
                PDesc: newPDesc
            };
        }
    ...
    addData: {
        ...
        customAdd: async (conn, body, modUsr) => {
            ...
            const duplicateResult = await exeQuery(conn, checkDuplicateQuery);
            if (duplicateResult[0]?.Count > 0) {
                throw new Error(`Diamond Grade Code "${trimmedPMCd}" already exists`);
            }

            // Generate PValue3 (max+1) from 8848EmrKc
            ...
            if (result.rowsAffected[0] === 0) {
                throw new Error('Failed to add Diamond Grade');
            }

            return {
                message: 'Diamond Grade added successfully',
                PMCd: trimmedPMCd,
                PDesc: trimmedPDesc,
                PValue3: nextPValue3.toString()
            };
    ...
    deleteData: {
        customDelete: async (conn, body, modUsr) => {
            ...
            if (result.rowsAffected[0] === 0) {
                throw new Error('Diamond Grade not found');
            }

            return {
                message: 'Diamond Grade deleted successfully'
            };
        }
    }
};